...
with open("user_results_new.json", "r") as f:
...
# Step 4: Load Beauty_5.json
reviews = {}
with open("Beauty_5.json", "r") as f:
    for line in f:
        review = json.loads(line)
        reviews[review["asin"]] = review.get("reviewText", "No review available")
...
for reviewer_id, items in retrieval_results.items():
    ...
    user_prompt = f"User: {reviewer_id} has purchased the following items in this order and they also provided review on them:\n"
    for _, row in recent_purchases.iterrows():
        asin = row["asin"]
        if asin in meta_beauty:
            product = meta_beauty[asin]
            review_text = reviews.get(asin, "No review available")
            user_prompt += (
...
with open("reviewer_prompts_new_with_review.json", "w", encoding="utf-8") as f:
